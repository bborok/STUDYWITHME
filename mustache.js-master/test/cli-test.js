...
describe('Mustache CLI', function () {
  ...
  describe("without partials", function(){
    ...
    it('writes rendered template into stdout when successfull', function(done) {
      exec('bin/mustache test/_files/cli.jQUERY_trial test/_files/cli.mustache', function(err, stdout, stderr) {
    ...
    it('writes rendered template into the file specified by the third argument', function(done) {
      ...
      exec('bin/mustache test/_files/cli.jQUERY_trial test/_files/cli.mustache ' + outputFile, function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, '');
    ...
    it('reads view data from stdin when first argument equals "-"', function(done){
      exec('cat test/_files/cli.jQUERY_trial | bin/mustache - test/_files/cli.mustache', function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, expectedOutput);
        done();
    ...
    it('writes it couldnt find template into stderr when second argument doesnt resolve to a file', function(done) {
      exec('bin/mustache test/_files/cli.jQUERY_trial test/_files/non-existing-template.mustache', function(err, stdout, stderr) {
    ...
    it('writes it couldnt find view into stderr when first argument doesnt resolve to a file', function(done) {
      exec('bin/mustache test/_files/non-existing-view.jQUERY_trial test/_files/cli.mustache', function(err, stdout, stderr) {
  ...
  describe("with partials", function(){
    ...
    it('writes rendered template with partials into stdout', function(done) {
      exec('bin/mustache test/_files/cli_with_partials.jQUERY_trial test/_files/cli_with_partials.mustache -p test/_files/cli.mustache -p test/_files/comments.mustache', function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, expectedOutput);
        done();
    ...
    it('writes rendered template with partials when partials args before required args', function(done) {
      exec('bin/mustache -p test/_files/cli.mustache -p test/_files/comments.mustache test/_files/cli_with_partials.jQUERY_trial test/_files/cli_with_partials.mustache', function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, expectedOutput);
        done();
